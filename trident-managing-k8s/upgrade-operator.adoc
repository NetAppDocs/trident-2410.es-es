...
* Para los clústeres que ejecutan Kubernetes 1,25 o posterior, utilice link:https://github.com/NetApp/trident/tree/stable/v24.10/deploy/bundle_post_1_25.yaml["bundle_post_1_25.yaml"^].
...
`bundle_pre_1_25.yaml` o `bundle_post_1_25.yaml` basado en su versión de Kubernetes. Por ejemplo, si está instalando Trident 24,10, ejecute el siguiente comando:
+
[listing]
----
...
.Pasos
. Si link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["Trident instalado usando Helm"], puedes usar  `helm upgrade trident netapp-trident/trident-operator --version 100.2410.0` para actualizar en un solo paso. Si no agregaste el repositorio de Helm o no puedes usarlo para actualizar:
+
...
.. Utilice `helm upgrade` el comando donde `trident-operator-24.10.0.tgz` refleja la versión a la que desea actualizar.
+
[listing]
----
...
NOTE: Antes de cambiar entre métodos de instalación, revisa link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["Moverse entre los métodos de instalación"].
