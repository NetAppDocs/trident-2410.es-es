...
WARNING: El `tridentctl` binario para Linux que se proporciona en el archivo zip del instalador es la versión probada y compatible. Tenga en cuenta que `macos` el binario proporcionado en `/extras` la parte del archivo zip no se ha probado ni se admite.
...
* Se ha añadido la capacidad de configurar y deshabilitar la reparación automática de iSCSI (link:https://github.com/NetApp/trident/issues/864["Problema n.o 864"]).
...
* Se corrigieron los mensajes de advertencia de ACP cuando ACP no está activado (link:https://github.com/NetApp/trident/issues/866["Problema n.o 866"]).
...
* Expansión de volumen fija si un nuevo tamaño solicitado es menor que el tamaño total del volumen para los controladores de almacenamiento ONTAP-nas y ONTAP-nas-FlexGroup (link:https://github.com/NetApp/trident/issues/834["Problema n.o 834"^]).
* Tamaño de volumen fijo para mostrar solo el tamaño utilizable del volumen durante la importación para controladores de almacenamiento ONTAP-nas y ONTAP-nas-FlexGroup (link:https://github.com/NetApp/trident/issues/722["Problema n.o 722"^]).
...
* Se ha añadido la capacidad para pausar el aprovisionamiento de un volumen cuando el backend está definido en estado suspendido por el usuario (link:https://github.com/NetApp/trident/issues/558["Problema n.o 558"^]).
...
*Kubernetes:* Se ha corregido la eliminación de daemonset para admitir actualizaciones de cero tiempo de inactividad (link:https://github.com/NetApp/trident/issues/740["Problema n.o 740"^]).
...
* Se ha corregido el manejo de errores en el código del controlador de almacenamiento (link:https://github.com/NetApp/trident/issues/816["Problema n.o 816"^]).
...
* Se corrigieron los permisos de rol de cluster de operador de Trident para estar sincronizados con los permisos de grupo (link:https://github.com/NetApp/trident/issues/799["Problema n.o 799"^]).
...
* Se solucionó el problema por el que el controlador Trident no podía cerrarse inmediatamente (link:https://github.com/NetApp/trident/issues/811["Problema n.o 811"]).
...
* Se ha corregido el error de compilación para la arquitectura s390 (link:https://github.com/NetApp/trident/issues/537["Problema n.o 537"]).
* Se solucionó el nivel de registro incorrecto durante las operaciones de montaje de volúmenes (link:https://github.com/NetApp/trident/issues/781["Problema n.o 781"]).
...
* Se ha agregado compatibilidad con los nodos arm64 (link:https://github.com/NetApp/trident/issues/732["Problema n.o 732"]).
* Se ha mejorado el procedimiento de apagado de Trident desactivando los servidores API en primer lugar (link:https://github.com/NetApp/trident/issues/811["Problema n.o 811"]).
...
**Kubernetes:** Los grupos de iniciadores con ámbito de respaldo ya no se crearán al configurar los controladores ONTAP-san y ONTAP-san-economy (link:https://github.com/NetApp/trident/issues/758["Problema n.o 758"]).
...
* Kubernetes: Se han añadido opciones para excluir la creación de políticas de seguridad de Pod para reparar las instalaciones de Trident a través de Helm (link:https://github.com/NetApp/trident/issues/794["Cuestiones #783, #794"^]).
...
* Mejora general de la utilización de recursos de control de acceso basado en roles deTrident (link:https://github.com/NetApp/trident/issues/757["Problema n.o 757"^]).
...
* Se ha solucionado el problema específico del backend de ONTAP creado mediante `credentials` el campo que no se conectaba durante la actualización de 22.07.0 (link:https://github.com/NetApp/trident/issues/759["Problema n.o 759"^]).
* **Docker:** Se ha corregido un problema que provocaba que el plugin de volumen de Docker no se iniciara en algunos entornos (link:https://github.com/NetApp/trident/issues/548["Problema n.o 548"^] y link:https://github.com/NetApp/trident/issues/760["Problema n.o 760"^]).
...
** Añadido soporte para link:https://docs.netapp.com/us-en/trident/trident-use/volume-share.html["uso compartido de volúmenes entre espacios de nombres"].
...
* Agregado link:https://docs.netapp.com/us-en/trident/trident-reco/security-luks.html["Compatibilidad con volúmenes cifrados LUKS"] para `ontap-san` y `ontap-san-economy` controladores de almacenamiento.
* Se ha agregado compatibilidad con nodos de Windows Server 2019.
* Se añade link:https://docs.netapp.com/us-en/trident/trident-use/anf.html["Compatibilidad con volúmenes SMB en nodos de Windows"] a través `azure-netapp-files` del controlador de almacenamiento.
...
IMPORTANT: Si va a actualizar desde una versión anterior de Trident y utiliza Azure NetApp Files, el ``location``parámetro config ahora es un campo singleton obligatorio.
...
* Se ha solucionado el problema de extracción de imágenes de soporte automático al establecer `imageRegistry` el indicador durante la instalación. (link:https://github.com/NetApp/trident/issues/715["GitHub Número 715"^])
...
* Para realizar una reclamación de espacio en línea para VP iSCSI, el sistema operativo subyacente del nodo de trabajo puede requerir que se pasen las opciones de montaje al volumen. Esto se aplica a las instancias de RHEL/RedHat CoreOS, que requieren `discard` https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/managing_file_systems/discarding-unused-blocks_managing-file-systems["opción de montaje"^] ; Asegúrese de que descarte mountOption se incluye en el[`StorageClass`^] para admitir el descarte de bloques en línea.
