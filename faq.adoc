...
Sí, Trident puede instalarse sin conexión. Consulta link:../trident-install/kubernetes-deploy.html["Obtenga información acerca de la instalación de Trident"^].
...
El instalador de Trident crea un ClusterRole de Kubernetes, que tiene acceso específico a los recursos PersistentVolume, PersistentVolumeClaim, StorageClass y Secret del clúster de Kubernetes. Consulta link:../trident-install/kubernetes-customize-deploy-tridentctl.html["Personalice la instalación trimentctl"^].



=== ¿Puedo generar localmente los archivos de manifiesto exactos que Trident utiliza para la instalación?

Puedes generar y modificar localmente los archivos de manifiesto exactos que Trident usa para la instalación, si lo necesitas. Consulta link:trident-install/kubernetes-customize-deploy-tridentctl.html["Personalice la instalación trimentctl"^].
...
Aunque no se aconseja, puede utilizar la misma SVM de back-end para dos instancias de Trident. Especifique un nombre de volumen único para cada instancia durante la instalación y/o especifique un parámetro único `StoragePrefix` en el `setup/backend.json` archivo. De este modo, se garantiza que no se utiliza el mismo FlexVol para ambas instancias.
