...
| `nfsMountOptions`  a| 
Control detallado de las opciones de montaje NFS; valor predeterminado es “-o nfsvers=3”. *Disponible solo para los `ontap-nas` conductores y `ontap-nas-economy`*. https://www.netapp.com/pdf.html?item=/media/10720-tr-4067.pdf["Consulte la información de configuración del host NFS aquí"^].
 a| 
`-o nfsvers=4`
...
| `tieringPolicy`  a| 
Política de organización en niveles que se debe utilizar, la opción predeterminada es `none`; `snapshot-only` para la configuración de SVM-DR anterior a ONTAP 9.5
 a| 
`none`
...
NOTE:  `ontap-nas-economy`El controlador no es compatible actualmente con Docker Swarm, porque Swarm no orquesta la creación de volúmenes en varios nodos.
