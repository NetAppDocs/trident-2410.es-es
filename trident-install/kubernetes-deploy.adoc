...
Ya sea que implementes manualmente o usando Helm, el operador Trident es una gran manera de simplificar la instalación y administrar dinámicamente los recursos de Trident. Incluso puedes link:../trident-install/kubernetes-customize-deploy.html["Personalice la implementación del operador de Trident"] usando los atributos en el `TridentOrchestrator` recurso personalizado (CR).
